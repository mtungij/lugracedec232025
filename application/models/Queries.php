...
public function insert_employee_permission($data)
{
    return $this->db->insert('employee_permissions', $data);
}
...
{
    $this->db->select('l.*');
    $this->db->from('employee_permissions ep');
    $this->db->join('system_links l', 'l.link_id = ep.link_id', 'inner');
    $this->db->where('ep.empl_id', $empl_id);
    $this->db->where('l.active', 1);
    $this->db->order_by('l.group_name', 'ASC');
    $this->db->order_by('l.link_name', 'ASC');
...
public function get_employee_permissions($empl_id)
{
    ...
    return array_column($rows, 'link_id');
}

public function update_employee_permissions($empl_id, $permissions)
{
    $this->db->where('empl_id', $empl_id)->delete('employee_permissions');
...
public function get_employee_link_ids($employee_id)
{
    ...
    $this->db->where('empl_id', $employee_id);
...
// application/models/Queries.php
public function get_all_permissions()
{
    // Assuming you have a table named 'permissions' with columns: link_id, group_name, link_name
    $this->db->order_by('group_name', 'ASC');
    $this->db->order_by('link_name', 'ASC');
    $query = $this->db->get('system_links');
    return $query->result();
}
...
public function insert_employee($data)
{
    ...
    return $this->db->insert_id(); // returns empl_id
}
...
public function get_totalLoanout($customer_id){
	$total_loan = $this->db->query("SELECT * FROM tbl_loans WHERE customer_id = '$customer_id' AND loan_status = 'out' ORDER BY loan_id DESC");
	  return $total_loan->row();
	   ...
	   public function get_depost_with_interest_principal($comp_id)
{
	$today = date('Y-m-d');
...
public function get_depost_with_monthly_interest_principal($comp_id)
{
	...
	public function get_all_actions()
{
	...
	public function get_backdatedcash_transaction($comp_id){
    $date = date("Y-m-d");
    ...
    $data = $this->db->query("
        ...
        LEFT JOIN tbl_customer c ON c.customer_id = pr.customer_id
        LEFT JOIN tbl_blanch b ON b.blanch_id = pr.blanch_id
        LEFT JOIN tbl_employee e ON e.empl_id = pr.empl_id
        LEFT JOIN tbl_account_transaction dat ON dat.trans_id = pr.trans_id
...
public function get_search_dataCustomer($customer_id){
  ...
  function fetch_loan_list($customer_id)
 {
  ...
  public function get_sum_depostLoan($loan_id){
	...
	public function get_sum_loandepost($loan_id){
    	$loan = $this->db->query("SELECT SUM(p.depost) AS depos FROM tbl_depost  p  WHERE p.loan_id = '$loan_id'  GROUP BY p.loan_id ");
    	 return $loan->row();
    }
...
public function get_total_pendingComp($comp_id){
    $data = $this->db->query("
        SELECT 
            b.blanch_name,
            c.f_name,
    ...
    foreach ($data->result() as $r) {
        ...
        // Group by blanch_name
        $results[$r->blanch_name][] = $r;
    }

    return $results; // Returns grouped by blanch_name
}
...
public function get_loan_empl_data($empl_id){
	...
	$data = $this->db->query("
		SELECT 
			SUM(pr.depost) AS total_received,
			SUM(pr.withdraw) AS total_withdrawal,
			c.f_name,
			c.m_name,
			c.l_name,
			l.restration,
			...
			pr.prev_id,
			pr.trans_id,
			at.account_name AS depost_account,
			pr.with_trans,
			wa.account_name AS with_account 
		FROM tbl_prev_lecod pr 
		JOIN tbl_loans l ON l.loan_id = pr.loan_id 
		JOIN tbl_customer c ON c.customer_id = pr.customer_id 
		LEFT JOIN tbl_account_transaction at ON at.trans_id = pr.trans_id 
		LEFT JOIN tbl_account_transaction wa ON wa.trans_id = pr.with_trans 
		WHERE pr.empl_id = '$empl_id' 
		AND pr.lecod_day = '$today' 
		GROUP BY pr.prev_id
	");
	return $data->result();
}
...
public function check_empl_privillage($position_id,$empl_id,$comp_id){
 ...
 public function get_previous_comp_loan_with($from, $to, $comp_id, $loan_status) {
    $data = $this->db->query("
        SELECT 
            b.blanch_name,
            c.f_name,
...
public function get_previous_comploan_with_total($from, $to, $comp_id, $loan_status) {
    $data = $this->db->query("
        SELECT 
            b.blanch_name,
    ...
    return $data->result(); // multiple rows, each per branch
}
...
return $query->result_array();
 ...
 public function get_total_outStandcomp($comp_id){
	...
	public function get_mobile_transaction_blanch($blanch_id) {
		$date = date("Y-m-d");
		$data = $this->db->query("
			SELECT 
				pr.prev_id,
			...
			LEFT JOIN tbl_blanch b ON b.blanch_id = pr.blanch_id 
			LEFT JOIN tbl_employee e ON e.empl_id = pr.empl_id 
			LEFT JOIN tbl_pay p ON p.loan_id = pr.loan_id
			LEFT JOIN tbl_account_transaction dat ON dat.trans_id = pr.trans_id  
			LEFT JOIN tbl_account_transaction wat ON wat.trans_id = pr.with_trans 
			WHERE pr.blanch_id = '$blanch_id' 
			AND date(p.date_data) = '$date' 
			AND LOWER(dat.account_name) != 'cash'
			ORDER BY pr.prev_id ASC
		");
		return $data->row();
	}
