...
class Admin extends CI_Controller {
  ...
  $payments=  $this->queries->get_depost_with_monthly_interest_principal($comp_id);
...
// 	              exit();
  ...
  public function get_interest()
  {
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $payments=  $this->queries->get_depost_with_interest_principal($comp_id);

    foreach ($payments as $row) {
      echo "Total Paid: " . $row['depost'] . "<br>";
      echo "Interest Paid: " . $row['interest_paid'] . "<br>";
      echo "Principal Paid: " . $row['principal_paid'] . "<br><hr>";
  }
  }
...
public function employee()
{
    ...
    // Fetch branches, positions, and employees
    $blanch = $this->queries->get_blanch($comp_id);
    $position = $this->queries->get_position();
    $employee = $this->queries->get_employee($comp_id);

    // Identify MANAGEMENT position_id
    $management_id = null;

    foreach ($position as $pos) {
        if (strtoupper(trim($pos->position)) === 'MANAGEMENT') {
    ...
    // Group system_links by group_name for easier display in view
    $grouped_links = [];
    ...
    $this->load->view('admin/employee', [
        ...
        'position' => $position,
        'employee' => $employee,
        'management_id' => $management_id, // For JS to show permissions card
...
public function create_employee()
{
    ...
    // 🔑 MANAGEMENT permission validation
    $position_id   = $this->input->post('position_id');
    $management_id = $this->input->post('management_id');
    $permissions   = $this->input->post('permissions');

    if ((int)$position_id === (int)$management_id) {
        if (empty($permissions)) {
            $this->form_validation->set_rules(
                'permissions[]',
                'System Access',
                'required',
                ['required' => 'Select at least one system access']
            );
        }
    }
    ...
    unset($data['permissions'], $data['management_id']);

    $data['password'] = sha1($data['password']);

    // 1️⃣ Insert employee
    $empl_id = $this->queries->insert_employee($data);

    // 2️⃣ Insert permissions
    foreach ($permissions as $link_id) {
        $this->queries->insert_employee_permission([
            'empl_id' => $empl_id,
            'link_id' => $link_id
        ]);
    }
    ...
    $this->session->set_flashdata(
        'massage',
        'Employee registered successfully. Password = 1234'
    );
   ...
   return true;
    ...
    public function view_aggrement($customer_id)
    {
      ...
      $compdata = $this->queries->get_companyData($comp_id);
        ...
        $html = $this->load->view('admin/loan_aggrement', ["customer" => $customer, 'compdata' => $compdata], true);
    
        // Create an instance of mPDF
        $mpdf = new \Mpdf\Mpdf();
        
        // Set the footer (optional)
        $mpdf->SetFooter('Generated By James developers');
        
        // Write the HTML content to the PDF
        $mpdf->WriteHTML($html);
        
        // Output the PDF with a specified file name
        $mpdf->Output('mkataba_wa_maombi.pdf', 'I'); // 'I' means output to the browser
    ...
    public function view_LoanCustomerData($customer_id,$comp_id){
       ...
       $loan_history = $this->queries->get_loan_history($customer_id);
         ...
         $inc_history = $this->queries->get_loanIncomeHistory($customer_id);
    	    ...
    	    // echo "</pre>";
    	    // exit();
    	$this->load->view('admin/view_loan_customer',['customer'=>$customer,'sponser_detail'=>$sponser_detail,'loan_form'=>$loan_form,'collateral'=>$collateral,'local_oficer'=>$local_oficer,'inc_history'=>$inc_history , "loan_history"=>$loan_history]);
    }
  ...
  $query = $this->db->query($sqldata);
   return true;
      ...
      $remain_balance = $loan_aproved - $sms_data;
            ...
            //   exit();
           //send sms function
           $massage ='Ndugu,'.' '.$full_name .' '.'Tunapenda Kukutaarifu Kuwa Mkopo wako Wa Tsh.'.number_format($loan_aproved) . ' '.'Umepitishwa'. ' '. $comp_name.' '.' Inakujari Mteja';
  ...
  public function restore_loan($loan_id)
{
  ...
  public function deleted_loans()
  {
    ...
    $this->load->view('admin/loan_withdrawal_date', [
      'data' => $data,
      'datas' => $datas,
      'blanch' => $blanch,
      'from' => $from,
  ...
  public function penalt_payments()
  {
    $this->load->model('queries');
		$comp_id = $this->session->userdata('comp_id');
		...
		//    print_r($income);
		//          exit();
		$this->load->view('admin/penalts_today',['income'=>$income,'detail_income'=>$detail_income,'total_receved'=>$total_receved,'customer'=>$customer,'blanch'=>$blanch]);
  }
	...
	$total_interest_loan = $this->queries->get_sum_loanwithdrawal_interest($comp_id);
     ...
     $mpdf->Output();
  	...
  	public function print_backdate_cash(){
	$this->load->model('queries');
	$comp_id = $this->session->userdata('comp_id');
   	$backdates = $this->queries->get_backdatedcash_transaction($comp_id);
	$compdata = $this->queries->get_companyData($comp_id);
	 $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8','format' => 'A4-L','orientation' => 'L']);
     $html = $this->load->view('admin/backdated_payments_pdf',['compdata'=>$compdata,'backdates'=>$backdates],true);
     $mpdf->SetFooter('Generated By Dev James');
     $mpdf->WriteHTML($html);
     $mpdf->Output();
	}
...
public function create_withdrow_balance($customer_id){
   ...
   //       print_r($sum_total_loanFee);
		 // exit();
          ...
          $massage = 'Umepokea Tsh. ' . number_format($remain_balance) . ' kutoka kwa ' . $comp_name . '. Tarehe: ' . $today . '. Tunakutakia urejeshaji mzuri wa mkopo wako.';
...
// $loan_data = $this->queries->get_loanDisbarsed($loan_id);
// $loan_data_interst = $this->queries->get_loanInterest($loan_id);
// $interest_rate = $loan_data_interst->interest_formular;
// $rate_type = $loan_data_interst->rate;
// $principal_balance = $loan_data->how_loan;
...
$days_remain = $this->queries->get_loan_active_customer($customer_id);
   ...
   if ($remain_days == 0) {
       ...
       $massage = 'Ndugu '.$full_name.', umelipa TSH '.number_format($new_balance).' leo '.$date.' - '.$comp_name.'. Mpokeaji: '.$role.'. Deni: TSH '.number_format($remain_loan).'. Leo ni mwisho wa mkataba wako.';
       ...
       $massage = 'Ndugu '.$full_name.' ' .$middle_name .' ' . $last_name.', umelipa TSH '.number_format($new_balance).' leo '.$date.'. Mpokeaji: '.$role.'. Deni: TZS '.number_format($remain_loan).'. Malipo yamechelewa, lipa haraka kuepuka hatua zaidi.';
   ...
   //        exit();
	     ...
	     $days_remain = $this->queries->get_loan_active_customer($customer_id);
          ...
          if ($remain_days == 0) {
              ...
              $massage = 'Ndugu '.$full_name.', umelipa TSH '.number_format($new_balance).' leo tarehe '.$date.' - '.$comp_name.'. Mpokeaji: '.$role.'. Deni ni TSH '.number_format($remain_loan).'.';
...
public function backdated()
{
  ...
  //  echo "<pre>";
    ...
    $total_zidi_today = $this->queries->get_total_zidi_today($comp_id);
 	   ...
 	   // print_r($sugu_wateja);
 	   //       exit();
 	$this->load->view('admin/Backdated_transaction',['backdates'=>$backdates,'paytoday'=>$paytoday,'total_zidi_today'=> $total_zidi_today,'sum_depost'=>$sum_depost,'sum_withdrawls'=>$sum_withdrawls,'blanch'=>$blanch,'sum_deducted'=>$sum_deducted,'sum_paid_penart'=>$sum_paid_penart,'account_deposit'=>$account_deposit,'default_list'=>$default_list,'toyal_default'=>$toyal_default,'withdrawal_account'=>$withdrawal_account,'total_code_no'=>$total_code_no,'deducted_fee'=>$deducted_fee,'penart_paid'=>$penart_paid,'miamala'=>$miamala,'total_miamala'=>$total_miamala,'hai_wateja'=>$hai_wateja,'sugu_wateja'=>$sugu_wateja]);
 ...
 public function remove_prepaid_deposit($dep_id){
    ...
    public function blanchwisepending()
    {
      $this->load->model('queries');
      $comp_id = $this->session->userdata('comp_id');
      $data['transactions'] = $this->queries->get_total_pendingComp($comp_id);
...
// }
  ...
  public function all_previous_income(){
	...
	public function create_income_detail(){
        ...
        $massage = 'Ndugu, ' .$first_name . ' ' .$midle_name . ' ' .$last_name . ' ' .'Umelipa faini ya Tsh.'. number_format($penart_paid) . ' '.$comp_name .' kwa msaada 0778126672';
  ...
  return $this->db->delete('tbl_deducted_fee',['loan_id'=>$loan_id]);	
    ...
    // }
       ...
       public function get_loan_code_resend($customer_id){
        ...
        $this->sendsms($phone,$massage);
        $this->session->set_flashdata('massage','Namba ya siri ya mkopo imetumwa kwa mteja tayari ikiwa hijafika resend tena');
        return redirect('admin/data_with_depost/'.$customer_id);
      }
...
public function teller_transaction(){
  $this->load->model('queries');
  $comp_id = $this->session->userdata('comp_id');
  $empl_oficer = $this->queries->get_empl_data_loan($comp_id);
  $total_deposit = $this->queries->get_total_deposit($comp_id);
