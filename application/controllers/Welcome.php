...
class Welcome extends CI_Controller {
   ...
   // user sing in
	...
	public function Employee_signin(){
        ...
        $this->load->model('queries');
        $links = $this->queries->get_employee_links($userexit->empl_id);

        $permissions = [];
        foreach ($links as $link) {
            $permissions[] = trim($link->link_name);
        }

        // 3️⃣ SAVE BOTH SESSION + PERMISSIONS
        $this->session->set_userdata($sessionData);
      ...
      // public 
       public function withdraw_automatic_loan($loan_id){
		...
		$today = date("Y-m-d", strtotime("+1 day"));
      	//   $today = date("Y-m-d");
      	  @$loans = $this->queries->get_sum_loandepost($loan_id);
