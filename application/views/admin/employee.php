...
<?php include('incs/side.php'); ?>
    ...
    <select name="position_id" id="position_select" class="form-control select2" required>
        <option value="">Select Position</option>
        <?php foreach ($position as $positions): ?>
            <option value="<?= $positions->position_id ?>">
                <?= htmlspecialchars($positions->position) ?>
            </option>
        <?php endforeach; ?>
    </select>
</div>
...
<div class="col-md-12 mt-4" id="management-access" style="display:none;">
    <div class="card">
        <div class="header bg-dark text-white">
            <h2>Management System Access</h2>
        </div>

        <div class="body">
            <?php foreach ($grouped_links as $group => $links): ?>
                <h5 class="text-uppercase text-info mt-3">
                        ...
                        <div class="col-md-6">
                            <label class="fancy-checkbox">
                                <input type="checkbox"
                                       name="permissions[]"
                                       value="<?= $link->link_id ?>">
                                <span><?= htmlspecialchars($link->link_name) ?></span>
                            </label>
                        </div>
                    <?php endforeach; ?>
                </div>
...
$(document).ready(function () {
    ...
    const managementId = <?= (int)$management_id ?>;
    const managementCard = $('#management-access');
    ...
    $('#position_select').on('change', function () {
        ...
        if (selectedId === managementId) {
            managementCard.slideDown();
        } else {
            managementCard.slideUp();
            managementCard.find('input[type=checkbox]').prop('checked', false);
        }
    });
